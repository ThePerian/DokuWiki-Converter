...
# max length equals 256 minus 1 for slash and minus 4 for extension
my $MAX_NAME_LENGTH = 251;
...
for my $file (@files) {
  ...
  $param{'datafolder'} = decode($param{'inencoding'}, $param{'datafolder'});
  # trim page name if it's absolute path is longer than 256 symbols
  $param{'filename'} = trimName($param{'datafolder'}, $param{'filename'});
...
=upload images
  my $pagename = $param{'filename'};
  $pagename = $1 if ($param{'filename'} =~ m/([^:]+)$/);
  print "pagename: $pagename\n";
  my @files;
	use Data::Dumper;
  find(sub {
	if ( /$pagename\_html_[^\.]+\.png$/ ) {
	  my $filename = $File::Find::name;
	  push( @files, $filename );
	}
  }, $CURRENTDIR);
  ...
  for my $file (@files) {
	...
	my $filestring = readFile($file);
	$file = $1 if ($file =~ m/([^\/\\]+)$/);
    print "uploading $file \n";
	
	$req = RPC::XML::request->new(
      'wiki.putAttachment',
      RPC::XML::string->new($file),
      RPC::XML::string->new(encode_base64($filestring)),
      RPC::XML::struct->new());
...
sub trimName {
  ...
  while ( measureName($folder.$file)>=$MAX_NAME_LENGTH ) {
    chop $file;
  }
